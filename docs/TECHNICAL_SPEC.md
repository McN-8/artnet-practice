...
| `InputSource` | Reader-input boundary | Subscribes typed input events and returns an unsubscribe function; manual, browser keyboard/touch, and visible browser-control adapters share this contract |
...
1. Continue the browser player target: define a whole-state image/audio readiness policy, audio unlock and mixing policy, complete renderer visual/accessibility coverage, and add responsive reader-control styling, lifecycle integration, and browser-gesture arbitration against the established input contracts.
