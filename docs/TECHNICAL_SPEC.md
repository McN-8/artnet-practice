...
- A `BrowserRenderer` may share a `BrowserAssetLoader` with the engine. When supplied, state and panel images wait for fetched `Blob`s, use scene-scoped object URLs rather than raw filenames, and expose `loading`, `fetched`, or `failed` in `data-asset-status`. Duplicate images in one scene share an object URL. A new state or renderer disposal revokes those URLs and ignores late responses from prior scenes. A failed fetch leaves the image without a source (black background or unpainted panel, with existing text/alternatives intact); there is no automatic retry or image-decode guarantee. Without a loader, the adapter retains direct filename-based image loading for simple previews.
...
- Image/audio decode and load-error handling beyond the current unpainted-image fallback, a renderer/audio-ready transition policy, bounded memory and eviction policy, priority queues, and telemetry. The browser image bridge has scene-scoped URL cleanup but does not enforce the version-1 memory budget.
...
1. Continue the browser player target: add explicit image decode/readiness and fallback policy, connect fetched audio to playback, complete renderer visual/accessibility coverage, and implement device-input adapters against the established contracts.
