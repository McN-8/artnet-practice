...
| `InputSource` | Reader-input boundary | Subscribes typed input events and returns an unsubscribe function; manual and browser adapters share this contract, with `BrowserReaderInput` coordinating browser sources |
...
A state's authored input-lock flag now initializes a separate runtime lock whenever that state activates. A positive `inputLockDuration` schedules deterministic release through the same lifecycle clock; a zero-duration lock remains locked for the state. Moving to another state cancels the abandoned unlock timer and initializes that state's own policy. Engine lock subscribers receive the current value immediately and subsequent changes, and all input paths consult the runtime value rather than mutating the authored state definition.
...
- `Renderer` is injectable. `PrototypeRenderer` preserves console diagnostics. `BrowserRenderer` is the first DOM adapter: it mounts into a caller-owned element, creates a fixed 1600×900 stage, uniformly scales and centers that stage within the element, and recalculates layout on resize (or by an explicit `layout()` call). The caller owns the host element's dimensions and calls `dispose()` when finished.
- The browser adapter draws a decorative state-background image, literal dialogue text, and timed panel images in background → panels → dialogue order. Panel descriptions become image alternatives; dialogue uses text content rather than HTML insertion. State rendering replaces the previous background/dialogue and clears previous panels. The engine, not CSS animation, owns reveal timing, so this subset appears immediately when dispatched and adds no motion for reduced-motion readers.
- A `BrowserRenderer` may share a `BrowserAssetLoader` with the engine. When supplied, state and panel images wait for fetched `Blob`s, use scene-scoped object URLs rather than raw filenames, and expose `loading`, `fetched`, `decoded`, or `failed` in `data-asset-status`. `fetched` means the source is assigned but the browser has not finished decoding; only `decoded` confirms paint readiness. The image decoder is injectable for deterministic tests and defaults to `HTMLImageElement.decode()`. Duplicate images in one scene share an object URL but decode independently. A new state or renderer disposal revokes those URLs and ignores late fetch/decode responses from prior scenes. Fetch or decode failure leaves the image without a source (black background or unpainted panel, with existing text/alternatives intact); there is no automatic retry or whole-state playback gate. Without a loader, the adapter retains direct filename-based image loading for simple previews without decode-status reporting.
...
- Rendering composition order is the reveal layout rectangle, then any applicable reusable group treatment, then the reveal's instance treatment. The initial browser adapter renders the reveal rectangle plus instance translation, scale, rotation, skew, flips, and opacity. It does not yet apply reusable group treatments, crop, mask, filter, outline, or deformation, and its camera-path, effect, and overlay hooks are no-ops. It is a minimal browser playback surface, not evidence of full visual fidelity or production accessibility acceptance.
...
- Extend the browser adapter to cover the remaining treatment and layer contracts, accessibility acceptance (including scalable dialogue and reader controls), responsive asset handling, and camera/effect/overlay rendering.
...
- `InputSource` subscriptions feed the engine's existing input path and can be explicitly unbound. Traditional page-turn inputs use that path without authored prompts. `BrowserKeyboardInputSource` is an optional browser adapter attached to a caller-owned, focusable reader host. It maps right/left/up/down arrows, Space, plus/equal, and Enter to the existing input types; Enter defaults to forward tap or can be configured as double tap where the authored interaction requires it. An optional target-ID provider supports targeted prompts. It prevents default handling only for recognized reader keys, ignores repeats, modified/composing keys, and interactive/editable descendants (including buttons), and removes its listener on disposal. It does not install a document-wide listener, infer actual double-tap timing, or create focus/reader controls; those remain application responsibilities.
- `BrowserTouchInputSource` is a separate optional `InputSource` bound to the caller-owned reader host. One-finger taps in the host's left or right half emit directional tap inputs; vertical movement of at least 40 CSS pixels within 500 ms emits up/down swipe input. Movement within 12 pixels counts as a tap when released before 500 ms; remaining within that distance for at least 500 ms emits one `hold` input on release, never both hold and tap. Ambiguous movement, moved long presses, cancellation, and mouse/pen events do not emit. A two-finger gesture emits one targeted pinch/zoom input after distance changes by both 24 CSS pixels and 20%; small changes and three-or-more-finger gestures do not emit. Interactive descendants retain their own events. Double-tap arbitration is opt-in through a host policy so ordinary page taps remain immediate. When enabled, a first tap waits 300 ms; a second tap within 24 CSS pixels, the same target, and that window emits only `doubleTap`. An unmatched tap emits after the window, and disposal cancels it. The injectable clock makes this deterministic. The adapter does not call `preventDefault`, allowing native browser behavior; hosts must decide whether story gestures are appropriate and apply a compatible `touch-action` policy. Hold is one-shot rather than continuous press state; continuous zoom scale and complete browser-gesture arbitration remain planned.
- `BrowserReaderControls` provides a separate optional visible-control `InputSource` using seven native text-labeled buttons (one per supported input type) in a named navigation landmark. Each button has at least a 44×44 CSS-pixel minimum target, works with native focus/keyboard activation, and can pass a target ID through an injected provider. `updateForState()` hides and disables actions not backed by a current matching prompt, all actions during a state input lock, and traditional previous/next actions at supplied page boundaries. The host calls it after navigation, target changes, and lock changes; without a supplied traditional page position, no implicit page action is shown. It appends into a caller-owned host without replacing its content; disposal removes only its controls and listeners. The "Inspect" button emits the existing one-shot `hold` input rather than implementing continuous press duration. These are functional input equivalents, not a finished responsive or WCAG-conformant UI; the host still owns placement, focus order, styling/contrast, and refresh timing.
- The engine exposes synchronous state-change and runtime-input-lock subscriptions; each immediately reports its current value, then subsequent changes until explicit unsubscription. `BrowserReaderInput.connect()` uses both contracts to bind its combined input once and automatically refresh controls, presentation mode, page index/count, touch double-tap eligibility, and lock availability during navigation and timed unlock. Reconnecting replaces its prior link, `disconnect()` removes only that link, and disposal disconnects before cleaning up all child adapters, generated controls, listeners, and pending touch arbitration. A target-selection change without state activation still requires an explicit `updateForState()` call. This is lifecycle coordination, not a complete player-shell owner.
...
1. Continue the browser player target: define a whole-state image/audio readiness policy, audio unlock and mixing policy, complete renderer visual/accessibility coverage, and add responsive reader-control styling plus selected-target refresh integration against the established input contracts.
