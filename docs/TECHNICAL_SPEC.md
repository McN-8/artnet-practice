...
- `BrowserReaderInput` coordinates the keyboard, touch, and visible-control adapters behind one `InputSource`, allowing one `Engine.bindInput()` call. Its `updateForState()` forwards state/presentation/page context to the controls and enables touch double-tap arbitration only when the current unlocked state has a matching untargeted or selected-target double-tap prompt. The target-ID provider is shared across all three paths. `dispose()` unsubscribes and disposes every child, removes the generated controls, cancels pending touch arbitration through touch disposal, and stops forwarding events. The host still calls `updateForState()` after relevant runtime changes and disposes both this coordinator and other browser subsystems when the player unmounts; this is lifecycle coordination, not a complete player-shell owner.
...
1. Continue the browser player target: define a whole-state image/audio readiness policy, audio unlock and mixing policy, complete renderer visual/accessibility coverage, and add responsive reader-control styling plus automatic state-refresh integration against the established input contracts.
