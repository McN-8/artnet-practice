...
- `BrowserReaderControls` provides a separate optional visible-control `InputSource` using seven native text-labeled buttons (one per supported input type) in a named navigation landmark. Each button has at least a 44×44 CSS-pixel minimum target, works with native focus/keyboard activation, and can pass a target ID through an injected provider. It appends into a caller-owned host without replacing its content; disposal removes only its controls and listeners. The "Inspect" button emits the existing one-shot `hold` input rather than implementing continuous press duration. These are functional input equivalents, not a finished responsive or WCAG-conformant UI; the host still owns placement, focus order, styling/contrast, which actions are available for the current story state, and reading-mode policy.
...
1. Continue the browser player target: define a whole-state image/audio readiness policy, audio unlock and mixing policy, complete renderer visual/accessibility coverage, and add hold/double-tap arbitration plus responsive state-aware reader controls against the established input contracts.
