...
- The engine exposes synchronous state-change subscriptions. A subscriber immediately receives the current state, then every state activated through normal start, transition, traditional navigation, or snapshot restoration; explicit unsubscription stops delivery. `BrowserReaderInput.connect()` uses that contract to bind its combined input once and automatically refresh controls, presentation mode, page index/count, and touch double-tap eligibility on navigation. Reconnecting replaces its prior link, `disconnect()` removes only that link, and disposal disconnects before cleaning up all child adapters, generated controls, listeners, and pending touch arbitration. Target selection or an input-lock mutation made without state activation still requires an explicit `updateForState()` call. This is lifecycle coordination, not a complete player-shell owner.
...
1. Continue the browser player target: define a whole-state image/audio readiness policy, audio unlock and mixing policy, complete renderer visual/accessibility coverage, and add responsive reader-control styling plus lock/target refresh integration against the established input contracts.
